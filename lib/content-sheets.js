...
class ContentSheets {
  ...
  authenticate() {
    if (
      ...
      throw new Error(
        'No credentials found as environment variables (GOOGLE_AUTH_CLIENT_EMAIL or GOOGLE_AUTH_PRIVATE_KEY).'
      );
    }
        ...
        .then(auth => {
          this.sheets.spreadsheets.get(
            ...
            (error, response) => {
              ...
              if (!s) {
                return reject(
                  new Error('Unable to locate sheet from ID: ' + sheet)
                );
              }
    ...
    grid.forEach(g => {
      ...
      if (g.Type && g.Type.toLowerCase() === 'number' && g.Value) {
        ...
        content[key] = g.match(/yes|true|1|^y/i)
          ? true
          : g.match(/no|false|0|^n/i) ? false : null;
      }
