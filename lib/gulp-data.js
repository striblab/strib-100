...
function fetchData(done, options = {}) {
  ...
  db.query(query, (error, results) => {
    ...
    let combined = _.map(_.groupBy(results, 'c_COID'), c => {
      ...
      company.officer = {};
      _.each(fields.o, f => {
        company.officer[_.camelCase(f)] = officer['o_' + f];
      });
      _.each(fields.s, f => {
        company.officer[_.camelCase(f)] = officer['s_' + f];
      });

      // Look to see if there is a logo file
      company.hasLogo = false;
      if (fs.existsSync(path.join(logoPath, company.coid + '.png'))) {
        company.hasLogo = true;
      }

      // Look to see if there is a ceo file
      company.officer.hasImage = false;
      if (
        company.officer.officerId &&
        fs.existsSync(path.join(ceoPath, company.officer.officerId + '.png'))
      ) {
        company.officer.hasLogo = true;
      }

      // Make sure category is standardized
      if (company.category.match(/health\s*care/i)) {
        company.category = 'healthcare';
      }
      else if (company.category.match(/social\s*serv/i)) {
        company.category = 'social-services';
      }
      else if (company.category.match(/^art/i)) {
        company.category = 'arts';
      }
      else if (company.category.match(/education/i)) {
    ...
    if (options.output) {
      ...
      fs.writeFileSync(options.output.replace('.json', '-simple.csv'), csv.format(_.map(combined, c => {
        return {
          coid: c.coid,
          company: c.company,
          officerName: c.officer.first + ' ' + c.officer.last,
          officerID: c.officer.officerId
        };
      })));
    }
